...
<template>
  <div class="w-full max-w-md mx-auto p-4 bg-white rounded-lg shadow-lg">
    <h3 class="text-lg font-bold text-gray-800 mb-4 text-center">Game Settings</h3>
    ...
    <div class="mb-4">
      ...
      >
        <option v-for="option in gameModeOptions" :key="option.value" :value="option.value">
          {{ option.label }}
        </option>
      </select>
    </div>
    ...
    <div class="text-sm text-gray-600 bg-gray-50 p-3 rounded-md">
      ...
      <p v-if="props.gameMode === 'human-vs-ai'">
        ...
        {{ difficultyOptions.find((opt) => opt.value === props.aiDifficulty)?.label }}
      </p>
    </div>
  </div>
</template>
